...
import {VStack, Box, Text, Button, FlatList, Center, Image} from 'native-base';
  ...
  const renderStockItem = ({item}) => (
    <Box
      ...
      <Text color="coolGray.800">
        {item.symbol} - {item.shares} Shares @ {item.price}
      </Text>
    </Box>
  );
  ...
  return (
    <VStack flex={1} bg="white" space={4} safeArea>
      <Center mb={4}>
        ...
        <Text fontSize="md" color="coolGray.800" py={2}>
          Your Portfolio Value: {userPortfolioValue}
        </Text>
      ...
      <VStack flex={1} width="100%">
        <Text fontSize="md" fontWeight="bold" color="purple.500" px={4}>
          Leaderboard
        </Text>
        <FlatList
